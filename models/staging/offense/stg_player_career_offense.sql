...
Model:          stg_player_career_offense
Author:         Tom Molitor
Created:        1/6/2025
Last Modified:  1/7/2025

Description: Staging table to bring in raw players career stats for futher analysis
...
SELECT 
    ...
    , CAST(intentional_walks                				AS INT) AS intentional_walks
    ...
    , CAST(grounded_into_double_plays       				AS INT) AS grounded_into_double_plays
    ...
    , CAST(fielded_by_battery               				AS INT) AS fielded_by_battery
    ...
    , CAST(batted_location_middle_infield   				AS INT) AS batted_location_middle_infield
    , CAST(batted_location_left_infield     				AS INT) AS batted_location_left_infield
    , CAST(batted_location_left_field       				AS INT) AS batted_location_left_field
    ...
    , CAST(batted_location_known            				AS INT) AS batted_location_known
    ...
    , CAST(stolen_base_opportunities_second      			AS INT) AS stolen_base_opportunties_second
    ...
    , CAST(outs_avoided_on_errors           				AS INT) AS outs_avoided_on_errors
    ...
    , CAST(bases_advanced_on_balls_in_play      			AS INT) AS bases_advanced_on_balls_in_play
    , CAST(surplus_bases_advanced_on_balls_in_play 			AS INT) AS surplus_bases_advanced_balls_in_play
    , CAST(outs_on_extra_base_advance_attempts  			AS INT) AS outs_on_extra_base_advance_attempts
    , CAST(pitches                              			AS INT) AS pitches
    ...
    , CAST(pitcher_pickoff_attempts             			AS INT) AS pickoff_attempts_pitcher
    ...
    , CAST(event_coverage_rate                  			AS FLOAT) AS event_coverage_rate
